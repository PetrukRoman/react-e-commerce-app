...
const Cart = () => {
  const { closeCart, isCartOpen } = useContext(ModalContext);
  const { items, subTotal } = useSelector((state) => state.cart);
  const dispatch = useDispatch();
  return (
    <Modal open={isCartOpen} closeModal={closeCart}>
      <div className={classes.cart}>
        <div className={classes["cart-info"]}>
          <header>
            <h2>Shopping Cart</h2>
            <ButtonIcon onClick={() => dispatch(resetCart())}>
        ...
        <div className={classes["cart-cta"]}>
